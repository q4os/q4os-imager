...
void ImageWriter::startWrite()
{
    ...
    _thread->setUserAgent(QString("Mozilla/5.0 q4os-imager/%1").arg(constantVersion()).toUtf8());
    //_thread->setUserAgent(QString("").toUtf8()); //or "curl/7.65.0", for sourcforge downloads
...
void ImageWriter::changeLanguage(const QString &newLanguageName)
{
    ...
    if (trans->load(":/i18n/q4os-imager_"+langcode+".qm"))
    {
