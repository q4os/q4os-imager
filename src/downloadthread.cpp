...
bool DownloadThread::_openAndPrepareDevice()
{
    ...
    if (authopenresult == _file.authOpenCancelled) {
        ...
        msg += "<br>"+tr("Please verify if 'Q4OS Imager' is allowed access to 'removable volumes' in privacy settings (under 'files and folders' or alternatively give it 'full disk access').");
...
void DownloadThread::run()
{
    ...
    if ( (_url.contains("sourceforge.net/projects/")) or (_url.contains("sf.net/projects/")) ) {
      setUserAgent(QString("").toUtf8()); //or "curl/7.65.0", for sourcforge downloads
      //qDebug() << "User agent modified as SourceForge detected.";
    }
    //qDebug() << "User agent: " << _useragent;

    char errorBuf[CURL_ERROR_SIZE] = {0};
    _c = curl_easy_init();
    //curl_easy_setopt(_c, CURLOPT_VERBOSE, 1);
    ...
    //curl_easy_setopt(_c, CURLOPT_AUTOREFERER, 1);
    curl_easy_setopt(_c, CURLOPT_FOLLOWLOCATION, 1);
    curl_easy_setopt(_c, CURLOPT_MAXREDIRS, 10);
    //curl_easy_setopt(_c, CURLOPT_POSTREDIR, CURL_REDIR_GET_ALL);
    ...
    switch (ret)
    {
        ...
        case CURLE_WRITE_ERROR:
            ...
            if (_file.errorCode() == ERROR_ACCESS_DENIED)
            {
                ...
                {
                    msg += "<br>"+tr("Controlled Folder Access seems to be enabled. Please add both q4os-imager.exe and fat32format.exe to the list of allowed apps and try again.");
                }
