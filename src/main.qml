...
ApplicationWindow {
    ...
    MsgPopup {
        ...
        text: qsTr("Q4OS Imager is still busy.<br>Are you sure you want to quit?")
        onYes: {
            Qt.quit()
        }
    }
    ...
    function selectDstItem(d) {
        if (d.isReadOnly) {
            onError(qsTr("Storage device is write protected.<br>If it has a physical lock switch, make sure it is unlocked, and try again."))
            return
        }
